...
const SITEMAP_CACHE_KEY = 'sitemap_btc_index_xml_cache';
...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  try {
    ...
    if (cachedSitemap) {
      console.log('Serving BTC tokens sitemap index from Redis cache');
    ...
    console.log('Generating fresh BTC tokens sitemap index and caching in Redis');
    ...
    for (let i = 1; i <= sitemapCount; i++) {
      sitemapEntries.push(`
        <sitemap>
          <loc>${escapeXml(`${domain}/sitemap/btc/tokens/${i}.xml`)}</loc>
    ...
    console.error('Error generating BTC tokens sitemap index:', error);
    return {
      props: {},
    };
  }
...
const BtcSitemapIndex = () => null;
export default BtcSitemapIndex;
